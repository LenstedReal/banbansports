...
function appendToken(u, token) {
  if (/[?&]token=/.test(u)) return u.replace(/([?&])token=[^&]*/, `$1token=${token}`);
...
async function serveMedia(request, env, url, token) {
  ...
  if (bucket) {
    ...
    if (!resp) return deny(404, 'Not Found', request, env);
  } else {
  ...
  if (!headers.get('Content-Type') || /octet-stream/.test(headers.get('Content-Type'))) {
    if (/\.ts$/i.test(url.pathname)) headers.set('Content-Type', 'video/MP2T');
    else if (/\.m4s$/i.test(url.pathname)) headers.set('Content-Type', 'video/iso.segment');
  }
