...
export default function FpsCounter() {
  ...
  // Mount'ta hide state'i localStorage'dan oku
  useEffect(() => {
    try { if (localStorage.getItem('bb_fps_hidden') === '1') setHidden(true); } catch { /* noop */ }
  ...
  const handleClick = () => {
    setHidden(true);
    try { localStorage.setItem('bb_fps_hidden', '1'); } catch { /* noop */ }
  };
  ...
  return (
    <div className={`fps-counter ${cls}`}
         ...
         title="FPS — tıkla gizle (geri açmak için console: bbToggleFps())">
