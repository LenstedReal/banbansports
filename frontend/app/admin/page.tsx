...
function AdminInner() {
  ...
  const [tab, setTab] = useState<'overview' | 'predictions' | 'users' | 'chat' | 'sponsors'>('overview');
  ...
  return (
    <div className="app-container" data-testid="admin-page">
      ...
      <nav style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '0 20px' }} data-testid="admin-tabs">
        {(['overview', 'predictions', 'users', 'chat', 'sponsors'] as const).map((t) => (
          <button
          ...
          >
            {t === 'overview' ? 'GENEL' : t === 'predictions' ? 'TAHMİNLER' : t === 'users' ? 'KULLANICILAR' : t === 'chat' ? 'SOHBET' : 'SPONSORLAR'}
          </button>
        ))}
      </nav>
