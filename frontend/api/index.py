...
# --- DB init (Vercel serverless cold start'ta bağlantıyı kur) ---
@app.on_event("startup")
async def _vercel_startup():
    if os.environ.get("MONGO_URL", "").startswith("mongodb"):
        try:
            from _backend_app.core.database import init_db
            await init_db()
        except Exception as e:  # pragma: no cover
            import logging
            logging.getLogger("vercel-slim").warning("DB init başarısız: %s", e)
...
for _path in (
    ...
    "_backend_app.routers.featured",        # öne çıkan yayın (tünel kaynak proxy)
    "_backend_app.routers.movies",          # film kataloğu + range'li stream
    "_backend_app.routers.boxoffice",       # canlı gişe + IMDb + seyirci sayacı
    "_backend_app.routers.stream_auth",     # korumalı film girişi — 30 dk signed token
    "_backend_app.routers.site_gate",       # site girişi — Turnstile "ben robot değilim" kapısı
