...
from .routers import scores, streams, bein, channels, ssport, stream_generic, auth as auth_router, predictions, chat, notifications, ws, match_stats, admin, push, ai_predict, internal, featured, movies, sponsors, boxoffice, stream_auth, site_gate
...
async def lifespan(app: FastAPI):
    ...
    movies_task = asyncio.create_task(movies.warm_cache())
    boxoffice_task = asyncio.create_task(boxoffice.refresh_loop())
    logger.info("banbansports v4 ready (env=%s)", "production" if IS_PRODUCTION else "development")
    try:
        yield
    finally:
        for t in (score_task, st11_task, st15_task, settle_task, movies_task, boxoffice_task):
            t.cancel()
            try:
                await t
